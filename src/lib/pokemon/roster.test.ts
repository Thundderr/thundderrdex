...
describe("getRoster", () => {
  ...
  it("includes megas (reordered) and regionals uniformly", () => {
    expect(byDisplay.has("Mega Charizard X")).toBe(true);
    expect(byDisplay.has("Raichu Alola")).toBe(true);
  });
  it("excludes cosmetic patterns, gender forms, and CAP", () => {
    expect(roster.some((e) => e.showdownName === "Vivillon-Fancy")).toBe(false); // cosmetic
    expect(roster.some((e) => e.showdownName === "Meowstic-F")).toBe(false); // gender → Phase 2
    expect(byDisplay.has("Meowstic")).toBe(true); // base kept
    expect(roster.some((e) => e.showdownName === "Crucibelle-Mega")).toBe(false); // CAP
    expect(roster.some((e) => /-Gmax$/.test(e.showdownName))).toBe(false);
  });
  it("flags Champions megas", () => {
    expect(byDisplay.get("Mega Pyroar")?.isChampionsMega).toBe(true);
  });
});
